...
Each day lives in its own folder (`day_1`, `day_2`, …) with its own `go.mod`, a small command-line entrypoint under `cmd/`, and an `internal` package that holds the core logic.
...
Replace `day_1` with `day_2`, etc., to run other days.
